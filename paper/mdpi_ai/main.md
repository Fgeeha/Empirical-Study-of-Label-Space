# An Empirical Study of Label-Space Alignment, Fair-Protocol Domain Adaptation and Edge Deployment for Field Plant Disease Recognition
...
Automated plant disease recognition has the potential to improve agricultural productivity, particularly in smallholder farming contexts where expert agronomists are unavailable [1,2]. Deep learning models trained on PlantVillage [3] achieve near-perfect accuracy under controlled, single-leaf-on-uniform-background conditions [4–6]. However, several practical barriers prevent direct deployment in field conditions [7,8].

**Label-space mismatch.** Agricultural image datasets are collected and annotated independently, resulting in incompatible class vocabularies. PlantVillage uses `Crop___Disease` labels; PlantDoc [9] uses different naming conventions. Before any cross-dataset evaluation can occur, a shared label vocabulary must be constructed.

**Visual domain shift.** Even for classes with matching labels, PlantVillage images are captured under controlled conditions, while field images include complex backgrounds, natural lighting variation, and occlusion [7,10]. A ResNet-50 trained on PlantVillage achieves macro-F1 = 0.969 on the PlantVillage test set but only 0.154 on PlantDoc over the same 18 classes — a drop of 0.81 in macro-F1 (Table 1).

**Edge deployment constraints.** Field deployment typically requires inference on battery-powered, low-cost hardware. Large models like ResNet-50 run at approximately 5 frames per second on a Raspberry Pi CPU, insufficient for real-time monitoring.

**What this paper does.** We do not propose a new training algorithm. Instead, we systematically study the design choices available to a practitioner trying to transfer plant disease recognition from PlantVillage to field conditions under a *fair*, protocol-audited evaluation (Figure 1). Specifically, we contribute:

1. **A controlled comparison of label-space alignment strategies** (fuzzy, Sentence-BERT (SBERT), hybrid) evaluated on the same shared class subset.
2. **A five-family unsupervised domain adaptation (UDA) benchmark** under a strict fair protocol: source-validation checkpoint selection, no target test labels used at any stage. We document three defects in our own earlier results — test-set checkpoint selection in the domain-adversarial (DANN) runs, an in-sample supervised upper bound, and a source-only baseline whose stored predictions did not match the archived checkpoints — and provide corrected numbers.
...
Post-training 8-bit integer (INT8) quantization [29,30] reduces model size and latency without retraining. The Google Coral Edge Tensor Processing Unit (Edge TPU) accelerates TensorFlow Lite (TFLite) INT8 models but imposes operator constraints [31]; architectures using Swish activations or squeeze-and-excitation blocks require CPU fallback. MobileNetV1/V2 [32,33] and EfficientNet-Lite (the mobile variant of EfficientNet [34], released as a model family without a separate publication) are designed for mobile and embedded inference; ResNet-50 [35] serves as the reference server-class architecture.

## 3. Materials and Methods

### 3.1. Datasets

**PlantVillage (source):** 38-class controlled-condition leaf disease dataset [3]; the segmented subset (single leaves on a uniform dark background) is used. After Hybrid alignment: 18 shared classes, 17,195 training images, 3,678 validation images, 3,701 test images. A fixed, class-stratified 10% sample of the training split (1,720 images, shuffled with seed 42, disjoint from validation and test) serves as the calibration set for post-training quantization.

**PlantDoc (primary target):** field-condition dataset [9]. The original publication describes 27 classes over 13 plant species; the released archive used here contains 28 class folders (the additional folder is `Tomato_two_spotted_spider_mites_leaf`). Hybrid alignment yields 18 shared classes; ten folders receive no mapping and are excluded (Section 3.3). All 1,969 aligned images form the target evaluation set. A class-stratified 80/20 split of this same list (seed 42) provides 1,568 images whose labels are discarded and which serve as the unlabeled adaptation set, and 401 images that are used only as the labeled early-stopping split of the supervised reference (Section 3.6). The adaptation images are therefore a subset of the evaluation images — the transductive UDA setting of the standard Office benchmarks [17] — whereas the PlantWild v2 evaluation (Section 4.4) is inductive: no PlantWild image is seen before testing.

**PlantWild v2 (secondary target):** 115-class web-scraped dataset (11,488 images) [11]. SBERT alignment at threshold 0.65 maps 46 of 115 classes (4,740 images) to the PlantVillage taxonomy; 17 of these classes overlap the 18-class output head of the Hybrid model (`Pepper,_bell___healthy` has no counterpart). Evaluation uses the 2,336 images of those 17 classes.

### 3.2. Problem Formulation

Let X denote the image space. We define:
- **Source domain:** D_S = {(x_i, y_i)}, labeled PlantVillage images, label space Y_S (38 classes).
- **Target domain:** D_T = {x_j}, unlabeled target images (train split for adaptation only).
- **Shared label space:** Y_ST ⊆ Y_S ∩ Y_T — constructed by label-space alignment.

Label-space alignment and feature-level adaptation are two distinct, sequential operations: alignment produces the class vocabulary; adaptation reduces the feature distribution gap. They are not jointly optimized.

### 3.3. Label-Space Alignment

All class names are normalized to lowercase, underscores replaced by spaces. Three strategies:

- **Fuzzy:** Token-set ratio matching [27] (threshold ≥ 80), yielding **16 aligned classes**.
- **SBERT:** Cosine similarity between `all-MiniLM-L6-v2` sentence embeddings [28] (threshold ≥ 0.75), yielding **15 classes**.
- **Hybrid:** Fuzzy primary, SBERT fallback, yielding **18 classes**.

Threshold stability (E1 sweep, Table S3): downstream macro-F1 is invariant for SBERT thresholds 0.50–0.75; only at 0.80 does F1 decrease. The 0.75 threshold is therefore a stable operating point, not a manually tuned optimum. For PlantWild v2, whose class names follow the same `{crop} {disease}` convention but include many crops absent from PlantVillage, we report alignment at 0.65, the lower end of the plateau, to maximize coverage; the downstream evaluation set is defined by the 17 overlapping classes and does not depend on this choice.
...
### 3.4. Fair Evaluation Protocol

**Training:** Models are trained on labeled source data D_S restricted to shared classes Y_ST.

**Adaptation (UDA):** Unlabeled target adaptation images (a subset of the evaluation set, Section 3.1) are used for feature alignment only. Target labels are never used at any stage of adaptation or checkpoint selection. Throughout, differences in macro-F1 are given in percentage points (pp; 1 pp = 0.01 macro-F1).

**Model selection:** Checkpoint selected by maximizing source-domain validation macro-F1 (3,678 PlantVillage images). This avoids any target information in model selection.

**Protocol audit:** A prior DANN result of ours (F1 = 0.277 ± 0.009) used target test labels for per-epoch checkpoint selection — a protocol violation. Under the fair protocol, DANN reaches F1 = 0.246 ± 0.005 (3 seeds; seed 42: 0.240). All results in this paper use the fair protocol; the legacy result is retained for transparency only.

### 3.5. UDA Methods Compared

- **AdaBN** [21]: Updates batch normalization running statistics from target images. No gradient updates.
- **CORAL** [15]: Aligns second-order feature covariance matrices. λ_CORAL = 0.1 (not tuned on target).
- **DANN** [17]: Unconditional adversarial alignment via gradient reversal.
- **CDAN+E** [19]: Conditional adversarial alignment on feature × prediction outer product with entropy conditioning.
- **MCC** [20]: Minimizes target class confusion (temperature T = 2.5).

### 3.6. Training Configuration

**Source-only models.** ResNet-50 [35] (ImageNet pretrained) is fine-tuned on the aligned PlantVillage training split with AdamW (lr = 3×10⁻⁴, weight decay 10⁻⁴), batch 32, cosine learning-rate schedule, up to 30 epochs with early stopping (patience 5) on validation macro-F1, input 224 × 224, ImageNet normalization, and training-time augmentation (horizontal flip, ±15° rotation, color jitter 0.2); seed 42. Table 1 and the source-only predictions used for the bootstrap tests are obtained by re-evaluating the archived checkpoints — the same files that initialized all UDA runs and the edge exports — with the evaluation transform of Section 3.4; the figures recorded at training time differ slightly and are discussed in Section 5.5.

**UDA models.** All five methods start from the same ImageNet-pretrained ResNet-50, use AdamW (lr = 3×10⁻⁴, wd = 10⁻⁴), 20 epochs, batch 32, seeds {42, 43, 44}, and identical data pipelines. Model selection: source-validation macro-F1. No target test labels are used at any stage. The loss implementations were unit-tested (20 tests, all passing).

**Adversarial methods.** Each source batch of 32 labeled images is paired with a target batch of 32 unlabeled images drawn cyclically from the PlantDoc training split (the target iterator restarts when exhausted); one epoch is one pass over the source loader. The domain discriminator is a three-layer multilayer perceptron (MLP) on the 2,048-dimensional pooled features (2048 → 1024 → 512 → 2, ReLU, dropout 0.5). The gradient reversal coefficient follows the standard schedule α = 2/(1 + exp(−10p)) − 1, where p ∈ [0, 1] is the fraction of training completed; the total loss is the source cross-entropy plus the unweighted domain cross-entropy on source and target batches. CDAN+E feeds the discriminator the outer product of features and softmax predictions (2,048 × 18), reduced to 2,048 dimensions by a random Gaussian projection matrix that is drawn once under the run seed (42, 43 or 44), stored as a non-trainable buffer and kept fixed, and weights each sample's domain loss by exp(−H(p)), the entropy-conditioning term. CORAL adds λ = 0.1 times the squared Frobenius distance between source and target feature covariances to the source cross-entropy; MCC uses temperature 2.5; AdaBN recomputes batch-normalization statistics on the target training images with no gradient step.

**Supervised reference.** ResNet-50 initialized from the source-only Hybrid model is fine-tuned on the 1,568 labeled PlantDoc adaptation images (the same images used, without labels, for adaptation), with the backbone frozen for the first 3 epochs, lr = 3×10⁻⁵, early stopping on the 401-image split, seed 42, a single run. Because the 1,568 training images belong to the 1,969-image evaluation set, a score on the full evaluation set is an in-sample figure (our earlier reports gave 0.852 on this basis; the archived checkpoint re-evaluates at 0.895 on the full list and at 0.985 on its own training images). We therefore report the held-out estimate on the 401 images the model never trained on: macro-F1 0.531 (accuracy 0.581). These 401 images also served for early stopping, so the estimate is mildly optimistic and, with 7–45 images per class, noisy; a supervised reference on a fully disjoint split requires a re-split and retraining and is left for future work.

**Edge models.** MobileNetV1, MobileNetV2, EfficientNet-Lite0, EfficientNet-B3 and ResNet-50 are each fine-tuned on the same aligned PlantVillage split with the source-only recipe above (18-class head, no domain adaptation), then quantized (Section 3.8). Their purpose is to measure deployment cost per architecture; their target-domain accuracy is not part of the benchmark.

**Compute.** Training ran on a single workstation GPU (NVIDIA GeForce RTX 5070 Ti, 16 GB) under Linux with Python 3.12 and PyTorch 2.x; one UDA run (20 epochs) takes about 20–25 min. Per-run configuration, git revision and seeds are stored alongside every result file.

### 3.7. Evaluation Metrics and Statistics

Primary: macro-F1. Secondary: accuracy, balanced accuracy. Statistical tests: paired bootstrap 95% confidence interval (CI) [36] of the difference in macro-F1 against source-only on the 1,969 PlantDoc test predictions of seed 42 (2,000 resamples). For the direct DANN versus CDAN+E comparison we use 10,000 resamples per seed to resolve the smaller differences. Multiple testing is not Bonferroni-corrected; five comparisons against source-only and three seed-wise comparisons are reported in full.

### 3.8. Edge Deployment Setup

**Hardware:** Raspberry Pi 4 Model B (8 GB RAM); Coral USB Edge TPU (USB 3.0).

**Software:** Edge TPU compiler v16.0.384591198. TFLite runtime v2.14.0 on Debian 12 (Bookworm), Python 3.11.2, `libedgetpu1-std` 16.0.

**Protocol:** 20 warm-up runs, 200 measured runs per model; batch = 1; 4 CPU threads; each timed run covers one interpreter invocation on a fixed synthetic input tensor (seed 42), so image decoding and preprocessing are excluded. Reported: mean latency, frames per second and model size (Table 5); latency standard deviation and p95 are given in Table S4. Model sizes are file sizes in MiB (2^20 bytes).
...
**Table 2.** UDA benchmark on PlantDoc (Hybrid 18 classes, fair protocol). Macro-F1: mean ± sample std over seeds 42/43/44. Δ (3 seeds): mean difference to source-only in percentage points (pp), computed from unrounded values (source-only = 0.1539). Δ (seed 42) and CI: point estimate and paired bootstrap 95% CI of the difference on seed-42 predictions (n = 1,969, 2,000 resamples); the two Δ columns have different bases and are not expected to coincide. All methods use a ResNet-50 backbone.
...
(1) **DANN achieves the highest mean macro-F1 on PlantDoc** (0.246 ± 0.005, +9.2 pp, bootstrap CI excludes zero). CDAN+E is close (0.242 ± 0.026, +8.8 pp). The mean difference between DANN and CDAN+E was 0.004 macro-F1, which is small relative to the across-seed variability of CDAN+E (std = 0.026).

(2) **DANN > CORAL on PlantDoc** (0.246 vs. 0.196; the earlier DANN macro-F1 of 0.277 was inflated by test-set checkpoint selection).

(3) **MCC collapsed** on all three seeds: the class-confusion objective caused source forgetting from the first epoch — source-validation macro-F1 never exceeded 0.28 in any run and the selected checkpoints score 0.056–0.098 on the target — confirming incompatibility with small, imbalanced target datasets at default hyperparameters.

(4) **AdaBN provides no benefit**: batch normalization statistics recalibration is insufficient for this domain gap.

**DANN vs. CDAN+E direct comparison.** Per-seed paired bootstrap (same n = 1,969 test samples, 10,000 resamples) gives inconsistent directionality: on seed 42 CDAN+E is significantly better (Δ = −0.027, CI [−0.046, −0.006]); on seed 43 DANN is significantly better (Δ = +0.035, CI [+0.017, +0.052]); seed 44 is inconclusive (Δ = +0.003, CI [−0.016, +0.022]). The three-seed mean difference (DANN − CDAN+E = +0.004) is small relative to CDAN+E's across-seed variability. Three seeds therefore do not support a population-level ranking of DANN and CDAN+E on this benchmark — the evidence is inconclusive rather than a demonstrated equivalence; DANN is, however, markedly more stable across seeds (std 0.005 vs. 0.026).

### 4.3. Fair Cross-Strategy Comparison

**Table 3.** Strategy comparison on the shared 13-class subset (n = 1,509, identical test samples for all strategies). The 13 classes are the intersection of the three class mappings; the rows re-score the re-evaluated predictions of the seed-42 source-only models (Section 3.6) and the stored predictions of the earlier single-seed CORAL runs (per-strategy λ from the sweep in Table S1, which was evaluated on the target split under the earlier protocol) on the shared classes only — no model was retrained. Single seed, so no standard deviation or confidence interval is available.
...
CORAL improves all three strategies on the shared subset by +2.3 to +3.8 pp. The spread between strategies (1.5 pp) is of the order of CORAL's seed-to-seed standard deviation in Table 2 (1.2 pp), so with one seed per cell the gains are consistent with being equal; the control shows that the CORAL gain is not an artifact of class-set size differences. It was run for CORAL only; the adversarial methods were trained in the Hybrid space alone, so their gains in Table 2 have no cross-strategy counterpart. Figure 3 shows t-distributed stochastic neighbor embedding (t-SNE) projections (perplexity 30, PCA initialization, seed 42) of the ResNet-50 features of the seed-42 models before and after CORAL adaptation for the Hybrid label space.

### 4.4. Generalizability: PlantWild v2

We evaluate the Hybrid model on PlantWild v2 (115 classes, 11,488 web-scraped images; 46 of 115 classes align with PlantVillage at threshold 0.65). Table 4 reports the fair-protocol results.

**Table 4.** PlantWild v2 generalization — fair-protocol results (source-validation checkpoint selection, 3 seeds each; the source-only row is the single seed-42 model). All methods are the 18-class Hybrid models adapted on PlantDoc unlabeled data; macro-F1 is computed over the 17 classes shared with PlantWild v2 (n = 2,336). No adaptation on PlantWild images was performed. Δ src is computed from unrounded values (source-only = 0.1722).
...
*DANN per-seed: 42 = 0.242, 43 = 0.226, 44 = 0.247. CDAN+E per-seed: 42 = 0.270, 43 = 0.195, 44 = 0.224.*

Both DANN and CDAN+E improve over the source-only baseline on PlantWild v2 even though neither model has seen a PlantWild image. DANN achieves a mean macro-F1 of 0.238 ± 0.011, compared with 0.230 ± 0.038 for CDAN+E. The mean difference (+0.85 pp for DANN, from unrounded means) is small relative to CDAN+E's seed variance; the ordering DANN ≥ CDAN+E and the stability advantage of DANN are the same as on PlantDoc.
...
**Table 5.** Edge deployment latency on Raspberry Pi 4 with Coral USB Edge TPU (200 timed runs after 20 warm-up runs, INT8 PTQ models, Hybrid 18-class heads). CPU: TFLite on the Raspberry Pi CPU with 4 threads. Size: INT8 TFLite file size in MiB (CPU file; the Edge TPU-compiled file differs by less than 0.5 MiB). FPS = frames per second, computed from the unrounded mean latency (e.g. 4.62 ms → 216 FPS). Measured files: the verified INT8 models of Section 3.8 (ONNX path; EfficientNet-Lite0 via the verified Keras path); each cell is one 200-run session, standard deviations and p95 are in Table S4. An earlier measurement of the first-generation files, three of which later proved to carry defective weights, gave 4.6 / 5.9 / 6.3 / 56.0 ms on the Edge TPU for the first four rows; the agreement confirms that latency is a property of the operator graph, not of the weight values.
...
MobileNetV1 achieves 4.6 ms (216 FPS) on the Edge TPU at 3.36 MiB, well within a real-time budget (Figure 4); MobileNetV2 and EfficientNet-Lite0 are within 2 ms of it. The adapted DANN model has the same operator graph as the source-only ResNet-50 and the same latency (54.9 versus 55.2 ms), so domain adaptation costs nothing at inference time. On the CPU the three small networks are within 3 ms of each other (30–33 ms), and ResNet-50 is about six times slower. EfficientNet-B3 cannot be delegated to the Edge TPU because Swish activations and squeeze-and-excitation blocks are not supported by the compiler [31].

### 4.6. Conversion Fidelity and INT8 Accuracy

INT8 PTQ reduces the ResNet-50 TFLite model from 89.7 MiB (FP32) to 23.1 MiB (3.9×). Whether the quantized models still recognize diseases is a separate question, which we answer by evaluating every converted model on the full test splits and comparing it with the PyTorch checkpoint it was derived from (Table 6).

**Table 6.** Macro-F1 of the converted models on the PlantVillage (PV, n = 3,701) and PlantDoc (PD, n = 1,969) test splits, Hybrid 18 classes. Reference: PyTorch FP32 of the same checkpoint evaluated with the same PIL preprocessing as the TFLite models (PIL bicubic resize). Elsewhere in the paper models are evaluated with the torchvision transform (bilinear resize with antialiasing), which differs by 1–2 pp: under that transform the source-only checkpoint gives 0.969 / 0.154 (Table 1) and DANN seed 42 gives 0.947 / 0.240, versus 0.974 / 0.148 and 0.961 / 0.249 here. The difference is the resize implementation, not the conversion. "Keras-rebuild" = the hand-written re-implementations behind the Table 5 latency measurements; "ONNX→TFLite" = the verifiable path of Section 3.8. Δ in pp relative to the reference. EdgeTPU: operations mapped by compiler 16.0.
...
Four findings follow. (i) **The first-generation exports were unusable for MobileNetV1, MobileNetV2 and EfficientNet-B3**: their INT8 files score at chance level on PlantVillage, and the defect reproduces when the current checkpoints are pushed through the same scripts, so it lies in the re-implementations, not in the weights. The Keras-rebuild ResNet-50 loses 2–3 pp because the Keras reference architecture places the stride-2 convolution in a different position than torchvision; EfficientNet-Lite0 was the only correct re-implementation. (ii) **With a faithful conversion, INT8 quantization is nearly free for the deployable architectures**: the float32 TFLite models reproduce the PyTorch predictions (top-1 agreement 0.99–1.00 on PlantVillage), and full-integer quantization lowers macro-F1 by at most 0.3 pp for MobileNetV1, MobileNetV2 and ResNet-50 on both test splits (full audit in Table S5) (the one larger change, +0.8 pp for MobileNetV2 on PlantDoc, is a gain within the noise of a 1,969-image test set). The adapted DANN model survives quantization equally well (PlantDoc 0.247 versus 0.249), so the domain-adaptation gain is preserved on the integer model that would actually be deployed. (iii) **EfficientNet-B3 does not quantize well** under post-training INT8 (−2.6 pp on PlantVillage, −11 pp on PlantDoc) and, being non-delegable, offers no advantage on this hardware. (iv) **The backbone may matter as much as the adaptation method.** The PyTorch column of Table 6 doubles as a source-only comparison of architectures on PlantDoc: EfficientNet-B3 reaches macro-F1 0.255 and EfficientNet-Lite0 0.212 without any adaptation, whereas the adapted ResNet-50 (DANN) reaches 0.249 under the same preprocessing (0.246 canonical). This is a single-seed, single-run observation without a confidence interval, whereas the DANN figure is a three-seed mean, so it is indicative rather than conclusive. The UDA benchmark of Section 4.2 is confined to ResNet-50, so this is not a contradiction of the adaptation gains, which are measured within one backbone, but it suggests that a better-pretrained, more modern backbone can recover as much of the field gap as adversarial adaptation does on ResNet-50. Combining the two was outside the scope of this study and is the most promising follow-up.

The four ONNX-path INT8 models compile with every operation mapped to the Edge TPU, and Table 5 reports their measured latency. Relative to the first-generation graphs, the verified MobileNetV1 and MobileNetV2 graphs differ by explicit padding operators (34 versus 29 and 69 versus 70 operations) and the verified ResNet-50 is the torchvision v1.5 variant rather than the Keras v1 graph; on the Edge TPU the differences are 0.0, −0.8 and −0.8 ms respectively, i.e. the verified files are as fast as or slightly faster than the defective ones, and the ranking is unchanged.

## 5. Discussion

### 5.1. What Helps

**Adversarial alignment gives the largest gains on both field targets.** DANN reaches 0.246 ± 0.005 (+9.2 pp) on PlantDoc and 0.238 ± 0.011 (+6.6 pp) on PlantWild v2; CDAN+E is close on both (0.242 ± 0.026, +8.8 pp; 0.230 ± 0.038, +5.7 pp). Both outperform CORAL (0.196 ± 0.012 on PlantDoc).

**DANN > CORAL under the fair protocol** on PlantDoc (0.246 vs. 0.196, both bootstrap-significant against source-only). DANN and CDAN+E cannot be separated with three seeds; DANN is the more stable of the two (std 0.005 vs. 0.026 on PlantDoc, 0.011 vs. 0.038 on PlantWild v2).
...
**AdaBN provides no benefit** (F1 = 0.155 vs. 0.154 source-only). Batch normalization statistics recalibration is insufficient for this domain gap.

**MCC failed** on all three seeds. The class-confusion objective caused source forgetting from the first training epoch on our small, imbalanced target dataset (1,568 images, 18 unbalanced classes). MCC is not suitable for this deployment scenario without significant hyperparameter adaptation.

**Label-alignment strategy does not strongly predict downstream UDA performance** on the shared 13-class subset. CORAL improves all three strategies by +2.3 to +3.8 pp (single seed per cell, earlier protocol), ruling out class-set-size artifacts.

### 5.3. Gap Analysis

The held-out supervised reference (macro-F1 0.531 on 401 images, Section 3.6) quantifies the gap to the best unsupervised result (DANN on PlantDoc: 0.246): about 0.29 in macro-F1, i.e. DANN recovers about a quarter of the distance from source-only (0.154) to the supervised reference. Among the evaluated methods under the present protocol, target macro-F1 remains below 0.25. The supervised reference uses PlantDoc labels and is not directly comparable to the UDA results; it is a single run on a small held-out split and should be read as an order of magnitude, not a precise ceiling. The in-sample figure of 0.852 reported in our earlier versions was measured on images the model had been trained on and is withdrawn.
...
For the practitioner the consequence is clear: at macro-F1 ≈ 0.25 over 18 classes, none of the unsupervised models is fit to act as a stand-alone field diagnostic. What the adapted models can support today is triage — flagging leaves for human or laboratory confirmation and prioritizing field images for annotation — and the latency results of Section 4.5 establish only that on-device compute is not the limiting factor for such a workflow. Closing the accuracy gap, not the latency budget, is the bottleneck for deployment.

### 5.4. Relation to the Companion Study

The companion study [12] and the present paper share the source dataset, the backbone and three of the adaptation methods, yet they reach different conclusions about adversarial adaptation: there, DANN and CDAN collapse to near-random performance in 67% and 80% of runs at the standard learning rate and CORAL is recommended as the default; here, DANN is the best-performing method with no collapsed run and CDAN+E is competitive. Three protocol differences separate the two experiments, and we state them rather than adjudicate between them. (The companion study is under review at the time of writing; its figures are quoted from the submitted manuscript, and its per-run outputs are included in the data deposit listed in the Data Availability Statement so that the comparison does not depend on its publication status.)

1. **Model selection.** The companion study evaluates a fixed training endpoint. Here the checkpoint is chosen on PlantVillage validation macro-F1. The per-epoch validation history of the adversarial runs is not monotone: for DANN seed 42 the source-validation macro-F1 drops to 0.31–0.44 at epochs 4–6 and again at epochs 19–20, and the selected checkpoint is epoch 7. A source-validation criterion therefore skips exactly the collapsed epochs that a fixed-endpoint protocol would report. This selection uses no target information and is legitimate under the fair protocol, but it explains why the same algorithm can appear unstable in one protocol and stable in another.
2. **Label space.** K = 28 classes there versus K = 18 here; macro-F1 over a larger class set with rarer classes is both harder and noisier.
3. **Seeds.** Five seeds there versus three here; three seeds cannot rule out an occasional collapse that five would reveal.
...
- **Single source domain:** All experiments use PlantVillage as source.
- **Label naming dependency:** The alignment pipeline requires `{crop} {disease}` naming. BeanDiseases (disease-only labels) yields 0 aligned classes.
- **Protocol correction:** Legacy DANN (F1 = 0.277 ± 0.009) used target-test checkpoint selection. Fair-protocol results (F1 = 0.246 ± 0.005) are the canonical numbers. All conclusions use the fair protocol.
- **Three seeds per method.** Means and standard deviations rest on three seeds; the companion study uses five. The bootstrap intervals in Table 2 describe sampling uncertainty over test images for one seed, not seed-to-seed variability.
- **MCC failure mode:** Default hyperparameters are unsuitable for small, imbalanced targets; a tuned MCC might behave differently.
- **Latency measurement.** Table 5 is one 200-run session per file on one device; run-to-run variation on the CPU was up to 9 ms for ResNet-50 (standard deviations in Table S4), and a repeated CPU session for the same ResNet-50 file differed by tens of milliseconds when the board had just finished another benchmark, so CPU figures should be read to about ±10%. Edge TPU figures were stable to within 0.6 ms.
- **INT8 accuracy was measured on the host.** The TFLite interpreter on x86 executes the same integer kernels, but Edge TPU execution of the four compiled models was not verified end to end.
- **Automatic label mappings.** The 18 class correspondences were constructed automatically and inspected by the author; they were not validated by an independent plant pathologist.
- **Transductive target protocol.** The unlabeled adaptation images are a subset of the PlantDoc evaluation set (Section 3.1). This is the usual UDA setting, and no target label is ever used, but the PlantDoc numbers are not inductive estimates; the PlantWild v2 results, obtained without seeing any PlantWild image, are.
- **Supervised reference.** The reference of 0.531 is one fine-tuning run (seed 42) evaluated on 401 held-out images that also served for early stopping. The 0.852 of our earlier versions was an in-sample score on the model's own training images and is withdrawn; a clean supervised reference needs a re-split of PlantDoc with a disjoint test set.
- **Single UDA backbone.** All five adaptation methods were run on ResNet-50 only; Table 6 shows that source-only EfficientNet-B3 already matches the adapted ResNet-50 on PlantDoc, so the ranking of methods may change with a stronger backbone.
- **Source-only checkpoint provenance.** The source-only figures recorded when the models were trained (PlantVillage / PlantDoc macro-F1 0.977 / 0.187 for Fuzzy, 0.975 / 0.152 for SBERT, 0.956 / 0.156 for Hybrid) do not reproduce from the archived checkpoints, and the stored Hybrid predictions agree with the re-evaluation on only 40% of PlantDoc images, whereas every fair-protocol UDA checkpoint reproduces its recorded score exactly; the source-only checkpoint files were evidently re-saved after those predictions were recorded. Because the archived checkpoints are the ones that initialized all UDA runs and the edge exports, Table 1, the shared-subset control (Table 3) and the bootstrap baseline of Table 2 use their re-evaluation (Hybrid 0.969 / 0.154); relative to the recorded figures this moves the PlantDoc baseline by −0.16 pp for Hybrid, −0.01 pp for Fuzzy and +1.85 pp for SBERT, and no conclusion changes. Both sets of predictions are included in the data deposit.

## 6. Conclusions

We presented a systematic empirical study of three practical bottlenecks in cross-dataset plant disease recognition — label-space mismatch, visual domain shift and edge-device constraints — using PlantVillage as source and PlantDoc and PlantWild v2 as field targets.

1. **Label alignment mainly determines class coverage, not adaptation quality.** Fuzzy, SBERT and hybrid matching yield 16, 15 and 18 shared classes; on a common 13-class subset CORAL improves all three by +2.3 to +3.8 pp (single seed per cell, earlier protocol), so the choice of strategy should be driven by coverage and naming conventions of the target dataset.
...
31. Google Coral. TensorFlow models on the Edge TPU: Model requirements. Available online: https://coral.ai/docs/edgetpu/models-intro/#model-requirements (accessed on 12 September 2026).
32. Howard, A.G.; Zhu, M.; Chen, B.; Kalenichenko, D.; Wang, W.; Weyand, T.; Andreetto, M.; Adam, H. MobileNets: Efficient convolutional neural networks for mobile vision applications. *arXiv* **2017**, arXiv:1704.04861. https://doi.org/10.48550/arXiv.1704.04861
33. Sandler, M.; Howard, A.; Zhu, M.; Zhmoginov, A.; Chen, L.-C. MobileNetV2: Inverted residuals and linear bottlenecks. In *Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)*, Salt Lake City, UT, USA, 18–23 June 2018; pp. 4510–4520. https://doi.org/10.1109/CVPR.2018.00474
34. Tan, M.; Le, Q.V. EfficientNet: Rethinking model scaling for convolutional neural networks. In *Proceedings of the 36th International Conference on Machine Learning (ICML)*, Long Beach, CA, USA, 9–15 June 2019; PMLR 97, pp. 6105–6114. https://doi.org/10.48550/arXiv.1905.11946
35. He, K.; Zhang, X.; Ren, S.; Sun, J. Deep residual learning for image recognition. In *Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR)*, Las Vegas, NV, USA, 27–30 June 2016; pp. 770–778. https://doi.org/10.1109/CVPR.2016.90
36. Efron, B.; Tibshirani, R.J. *An Introduction to the Bootstrap*; Chapman & Hall/CRC: Boca Raton, FL, USA, 1994. https://doi.org/10.1201/9780429246593
...
**Figure 3.** t-SNE projection (scikit-learn, perplexity 30, PCA initialization, learning rate "auto", random state 42) of ResNet-50 penultimate-layer features on the PlantDoc evaluation set (Hybrid label space, one color per class): (a) source-only model (seed 42); (b) after CORAL adaptation (λ = 0.1, seed 42). CORAL tightens several class clusters but substantial overlap remains, consistent with the modest macro-F1 gain in Table 2. File: `figures/figure3_tsne_hybrid.png` (PNG, 600 dpi).

**Figure 4.** Edge deployment on Raspberry Pi 4 with Coral USB Edge TPU (verified INT8 models, Hybrid heads, 200 timed runs). (a) Latency per image on the Raspberry Pi CPU and on the Edge TPU (log scale); EfficientNet-B3 cannot be delegated to the Edge TPU. (b) INT8 model size versus Edge TPU throughput. File: `figures/figure4_edge.png` (PNG, 600 dpi).
