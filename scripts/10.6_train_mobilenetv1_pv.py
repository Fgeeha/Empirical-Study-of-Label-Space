"""
10.6 Train MobileNetV1 on PlantVillage (PyTorch / timm, GPU).

Fine-tunes timm `mobilenetv1_100` (ImageNet pretrained) on PlantVillage
with the specified label-alignment strategy.

MobileNetV1 uses only DepthwiseSeparableConv + ReLU6 -- fully EdgeTPU
compatible (no Swish, no SE).
...
Usage:
    ...
    poetry run python scripts/10.6_train_mobilenetv1_pv.py --strategy Fuzzy
    poetry run python scripts/10.6_train_mobilenetv1_pv.py --strategy sbert
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Fine-tune MobileNetV1 (timm) on PlantVillage'
    )
    parser.add_argument(
    ...
    print(f'[INFO] Training MobileNetV1 for strategy={strategy}')
    ...
    for epoch in range(cfg['training']['epochs']):
        ...
        if f1 > best_f1:
            best_f1 = f1
            patience_counter = 0
            save_path = MODELS / f'mobilenetv1_pv_{strategy}_best.pt'
    ...
    print(f'[OK] MobileNetV1 training done, best_f1={best_f1:.4f}')
