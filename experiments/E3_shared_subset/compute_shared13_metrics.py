    ...
    for _, row in df.iterrows():
        for col in df.columns:
            ...
            if val in shared_names:
                ...
                id_col = [c for c in df.columns if "label" in c.lower() or "id" in c.lower()]
                if id_col:
...
def compute_metrics_shared(
    ...
    if name_col is None:
        print(f"  [ERROR] Cannot find class-name column in label_map. Cols: {lmap_cols}")
        return None
    ...
    if id_col is None:
        print(f"  [ERROR] Cannot find label_id column in label_map")
        return None
...
configs = [
    ...
    ("Fuzzy",  "Baseline (zero-shot)",
     RESULTS / f"preds_target_resnet50_Fuzzy{_SFX}.csv"),
    ("Fuzzy",  "CORAL λ=1.0",
     RESULTS / "preds_target_coral_lambda_1.0_Fuzzy.csv"),
    ("hybrid", "Baseline (zero-shot)",
     RESULTS / f"preds_target_resnet50_hybrid{_SFX}.csv"),
    ("hybrid", "CORAL λ=0.1",
     RESULTS / "preds_target_coral_lambda_0.1_hybrid.csv"),
    ("sbert",  "Baseline (zero-shot)",
     RESULTS / f"preds_target_resnet50_sbert{_SFX}.csv"),
    ("sbert",  "CORAL λ=0.01",
     RESULTS / "preds_target_coral_lambda_0.01_sbert.csv"),
]
...
# Write log
log_lines = [f"E3 run at {manifest['timestamp']}", f"Git: {manifest['git_hash']}",
             f"Shared classes: {manifest['n_shared_classes']}", df_out.to_string()]
