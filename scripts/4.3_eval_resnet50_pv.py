...
def main():
    ...
    out_json = RESULTS / f'resnet50_pv_metrics_{strategy}.json'
    out_json.write_text(
    ...
    pd.DataFrame(report).transpose().to_csv(
        RESULTS / f'classification_report_pv_{strategy}.csv'
    )
    ...
    plt.title(f'ResNet50 PV ({strategy})')
    plt.tight_layout()
    plt.savefig(FIGURES / f'cm_resnet50_pv_{strategy}.png')
    plt.close()

    print(f"[OK] Evaluation completed for strategy='{strategy}'")
